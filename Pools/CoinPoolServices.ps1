...
try {
    $Pool_Request = Invoke-RestMethodAsync "http://coinpoolservices.com/api/status" -tag $Name -cycletime 120
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://coinpoolservices.com/api/currencies" -tag $Name -cycletime 120 -delay 500
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
...
$Pool_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | ForEach-Object {
    $Pool_Host = "coinpoolservices.com"
